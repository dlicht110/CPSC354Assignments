...
ifAction boolCalc action1 action2 = do
  bo <- boolCalc
  if bo then action1 else action2
...
boolCalc (Reln cOp expr1 expr2) = do
  ...
  return fromMaybe (error "This should never happen")
    (lookup cOp relnopTable)
    $ c1 c2
...
boolCalc (BBin op expr1 expr2) =  do
  ...
  return fromMaybe (error "This should never happen")
    (lookup op boolBopTable)
    $ c1 c2

boolCalc (BUn op expr) = fmap (lookup op boolUnopTable) (boolCalc expr)
...
intCalc (ABin op expr1 expr2) = do
  ...
  return fromMaybe (error "This should never happen")
    (lookup op intBopTable)
    $ c1 c2


intCalc (AUn op expr) = fmap (lookup op intAUnopTable) (intCalc expr)
